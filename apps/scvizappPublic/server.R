...
my_cols <- c('3'='#F68282','15'='#31C53F','5'='#1FA195','1'='#B95FBB','13'='#D4D915',
             '14'='#28CECA','9'='#ff9a36','8'='#2FF18B','11'='#aef','6'='#faf4cf',
...
dotplot_with_genotype <- function(seurat_obj,
  ...
  g1 <- Seurat::DotPlot(
    ...
    ggplot2::theme(
      axis.text.x = ggplot2::element_text(
        color = "black",
        size = dotPlotXtextSize,
        angle = fontangle_x,
        vjust = 0.5
      ),
      axis.text.y = ggplot2::element_text(
        color = "black",
        size = dotPlotYtextSize
      ),
      axis.title = ggplot2::element_blank(),
      legend.text = ggplot2::element_text(
        color = "black",
        size = dotPlotLtextSize
      )
    )
  ...
  g2 <- ggplot2::ggplot(marker_genes_df) +
    ...
    ggplot2::theme(
      ...
      legend.text = ggplot2::element_text(color = "black", size = dotPlotLtextSize),
...
shinyServer(function(input, output, session) {
  ...
  observeEvent(input$rdsUploadSubmit, {
    ...
    tryCatch({
      ...
      file_name <- input$RDSFile$name
      file_path <- input$RDSFile$datapath
